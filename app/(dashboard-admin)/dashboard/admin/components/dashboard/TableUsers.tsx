...
import EditUserForm from '../../informacion/usuarios/EditUserForm';
import ViewUser from '../../informacion/usuarios/ViewUser';
...
interface TableUsersProps {
  usuarios: any;
  onUpdateUser: (onUpdateUser: any) => void
}
...
export default function TableUsers({ usuarios, onUpdateUser }: TableUsersProps) {
  ...
  const [viewingUser, setViewingUser] = React.useState<null>(null); // Para visualizar
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelected = usuarios.map((n: any) => n.user_id);
  ...
  const filteredUsers = usuarios.filter((usuario: any) => {
    const searchFilter =
